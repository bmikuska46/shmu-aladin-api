...
var englishDocsReplacer = strings.NewReplacer(
	...
	`Predpoveď podľa súradníc`, `Forecast by coordinates`,
	`Predpoveď`, `Forecast`,
	...
	`Najnovšia predpoveď ALADIN na nasledujúce 3 dni. Zadajte buď <code class="inline-code">station</code> (ID stanice), alebo <code class="inline-code">lat</code> a <code class="inline-code">lon</code> (najbližšia stanica). Voliteľný parameter <code class="inline-code">cnt</code> určuje počet hodinových krokov.`, `The latest ALADIN forecast for the next 3 days. Provide either <code class="inline-code">station</code> (station ID), or <code class="inline-code">lat</code> and <code class="inline-code">lon</code> (nearest station). The optional <code class="inline-code">cnt</code> parameter controls the number of hourly steps.`,
	...
	`Počet hodinových krokov v odpovedi.`, `Number of hourly steps in the response.`,
	...
	`curl · najbližšia stanica`, `curl · nearest station`,
